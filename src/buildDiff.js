...
const buildDiff = (object1, object2) => {
  ...
  const diff = keys.map((key) => {
    ...
    if (_.isPlainObject(firstValue) && _.isPlainObject(secondValue)) {
      return { key, status: 'nested', children: buildDiff(firstValue, secondValue) };
    }

    if (!hasSecondObjectKey) {
      return { key, value: firstValue, status: 'removed' };
    }
    ...
    if (!_.isEqual(firstValue, secondValue)) {
      return {
        key, value: secondValue, oldValue: firstValue, status: 'updated',
      };
    }

    return { key, value: firstValue, status: 'unchanged' };
  });
